...
def get_query_top_k(query, query_embeddings, docs, docs_embeddings, max_n=10, top_k=5, min_p=0.7):
	count=0
	top_k_list = []
	for score, idx in similarity(query_embeddings, docs_embeddings, max_n=max_n, top_k=top_k):
		score = score.item()
		if count<top_k and  score>min_p:#score<=0.99 and score>min_p: # we skip exact match, because of floating point precision we set exact match to 0.99
			count=count+1
